...
import { PRICE_SCALE, WEIGHT_LIMIT, WEIGHT_WINDOW_MS, WEIGHTS } from './exchange.constants';
import type {
  AssetBalance,
  ExchangeConfig,
  OrderBook,
...
export class ExchangeClient {
  ...
  async fetchOrderBook(symbol: string, limit: number = 20): Promise<OrderBook> {
    await this.checkWeight(WEIGHTS.orderBook);
    ...
    this.recordWeight(WEIGHTS.orderBook);
    ...
    console.log(
      `[ExchangeClient] orderBook ${symbol} bid=${bestBid[0]} ask=${bestAsk[0]} spread=${spreadBps}bps`,
    );
  ...
  async fetchOrderStatus(orderId: string, symbol: string): Promise<OrderResult> {
    await this.checkWeight(WEIGHTS.fetchOrder);
    ...
    this.recordWeight(WEIGHTS.fetchOrder);
  ...
  async getTradingFees(symbol: string): Promise<TradingFees> {
    await this.checkWeight(WEIGHTS.tradingFees);
    ...
    this.recordWeight(WEIGHTS.tradingFees);
  ...
  private async checkWeight(weight: number): Promise<void> {
    ...
    if (used + weight < WEIGHT_LIMIT) return;

    const oldest = this.weightLog[0];
    if (!oldest) return;

    const sleepMs = WEIGHT_WINDOW_MS - (Date.now() - oldest.time) + 50;
    if (sleepMs > 0) {
      console.warn(
        `[ExchangeClient] weight budget tight (${used}/${WEIGHT_LIMIT}) — sleeping ${sleepMs}ms`,
      );
